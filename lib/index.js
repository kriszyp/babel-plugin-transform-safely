module.exports = function ({ types: t }) {
  ...
  function ensureObject(path) {
    ...
    if (t.isIdentifier(node)) {
      ...
      path.replaceWith(t.logicalExpression('||', node, t.assignmentExpression('=', node, t.objectExpression([]))))
      return path.node
      ...
      path.replaceWith(t.logicalExpression('||',
        ...
        markAsSafe(t.assignmentExpression('=', markAsSafe(t.memberExpression(objectRef, node.property, node.computed)), t.objectExpression([])))))
  ...
  const safelyVisitors = {
    AssignmentExpression(path) {
      ...
      if (t.isMemberExpression(left)) {
        ensureObject(path.get('left').get('object'))
        markAsSafe(left)
//        else
